...
def test_dir_preset():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert dp.rootpath == test_rootpath
        assert dp.rootpath.exists()
        assert dp.test_dir == test_rootpath.joinpath("test_dir")
        assert dp.test_dir.exists()
        assert dp.test_file == test_rootpath.joinpath("test_file")
        assert dp.test_file.exists()
        ...
        # validate again with create=True, and backup=False,
        # which errors because the directory is dirty
        with pytest.raises(
            PathValidationError,
            match="invalid path: .+ exists or is not empty",
        ):
