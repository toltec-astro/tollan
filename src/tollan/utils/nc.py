...
def ncinfo(source):
    ...
    with ncopen(source) as nc:
        ...
        dimnames = [
            (dimname, len(nc.dimensions[dimname])) for dimname in nc.dimensions.keys()
        ]
        varnames = [
            (
                varname,
        ...
        grpnames = [(grpname,) for grpname in nc.groups.keys()]
      ...
      {head}
        ...
        grps: {grps}
            ...
            nc.filepath(),
            head=head,
            atts=tabulate(attnames, headers=["name", "value"]),
            dims=tabulate(dimnames, headers=["name", "ndims"]),
            vars_=tabulate(varnames, headers=["name", "info", "dtype" "description"]),
            grps=tabulate(grpnames, headers=["name"]),
        )
    return info
